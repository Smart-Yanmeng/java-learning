...
public class ElementDeserializerFromZr extends JsonDeserializer<Element> {

    @Override
    public Element deserialize(JsonParser p, DeserializationContext context) throws IOException {

        // 反序列化为 Element 对象
        return PairingParameter.getInstance().getZr().newElementFromBytes(p.getBinaryValue()).getImmutable();
    }
}
